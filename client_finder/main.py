...
def print_menu():
    from client_finder.config import is_warp_installed, get_warp_status
    ...
    print()
    print(f"  {Colors.YELLOW}[4]{Colors.RESET}  {Colors.BOLD}View Stats{Colors.RESET}       - Check email open rates")
    print(f"  {Colors.YELLOW}[5]{Colors.RESET}  {Colors.BOLD}Check Opens{Colors.RESET}      - See who opened your emails")
    print()

    warp_installed = is_warp_installed()
    if warp_installed:
        warp_status = get_warp_status()
        if warp_status == "Connected":
            warp_label = f"{Colors.GREEN}ON{Colors.RESET}"
        else:
            warp_label = f"{Colors.RED}OFF{Colors.RESET}"
        print(f"  {Colors.MAGENTA}[6]{Colors.RESET}  {Colors.BOLD}WARP VPN{Colors.RESET}        - Bypass network blocks [{warp_label}]")
    else:
        print(f"  {Colors.DIM}[6]  WARP VPN        - Install from https://1.1.1.1/{Colors.RESET}")

    print()
    print(f"  {Colors.CYAN}[7]{Colors.RESET}  {Colors.BOLD}Website List{Colors.RESET}     - Crawl your own list of websites")
    print(f"  {Colors.MAGENTA}[8]{Colors.RESET}  {Colors.BOLD}Telegram Bot{Colors.RESET}     - AI chatbot on Telegram")
    print(f"  {Colors.GREEN}[9]{Colors.RESET}  {Colors.BOLD}Google Maps{Colors.RESET}      - Serper API (direct Google data)")
    print()
...
def run_interactive():
    ...
    while True:
        ...
        if choice == "1":
            ...
            from client_finder.chatbot import start_bot
            start_bot()
        elif choice == "9":
...
def run_serper_interactive():
    ...
    from client_finder.sender import send_emails
    result = send_emails(to_send, MAX_EMAILS_TO_SEND)

    # Track
    from client_finder.tracker import mark_sent, check_opens
    for r in result.get("results", []):
        if r["status"] == "sent":
            mark_sent(r["email"], r["subject"], r.get("name", ""))
    check_opens()
